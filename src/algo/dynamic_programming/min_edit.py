...
def min_edit_distance(source: str, target: str):
    ...
    # Target is empty, the cost of deleting every source character.
    ...
    for i in range(1, n+1):
        for j in range(1, m+1):
            ...
            above = dp[i-1][j] + 1
            left = dp[i][j-1]+1
            corner = dp[i-1][j-1] + sub_cost
            
            dp[i][j] = min(above, left, corner)

    return dp

def print_min_edit_distance(dp: List[List[int]], source: str, target: str):
    # 1. 定义列宽，保证对齐
    header_col_width = 7
    col_width = 2
...
def min_edit_distance_structured(source: str, target: str):
...
if __name__ == "__main__":
    ...
    print_min_edit_distance(result, source, target)

    dp, parent = min_edit_distance_structured(source, target)
